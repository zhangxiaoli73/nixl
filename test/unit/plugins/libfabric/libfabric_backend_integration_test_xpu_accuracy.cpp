...
void levelZeroMemCpy(void* src, void* dst, size_t total_size, ze_device_handle_t device) {
    ...
    auto ret = zeCommandQueueCreate(global_context, device, &cq_desc, &cmdq);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandQueueCreate failed\n";
        return;
    }

    ret = zeCommandListCreate(global_context, device, &cl_desc, &cmdl);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandListCreate failed\n";
        return;
    }

    ret = zeCommandListAppendMemoryCopy(cmdl, dst, src, total_size, NULL, 0,
                                        NULL);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandListAppendMemoryCopy failed\n";
        return;
    }

    // must close commandlist before execute
    ret = zeCommandListClose(cmdl);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandListClose failed\n";
        return;
    }

    ret = zeCommandQueueExecuteCommandLists(cmdq, 1, &cmdl, NULL);
    if (ret != ZE_RESULT_SUCCESS) {
...
//    ret = zeCommandListReset(cmdl);
//    if (ret != ZE_RESULT_SUCCESS) {
//        std::cerr << "zeCommandListReset failed\n";
//        return;
//    }

    ret = zeCommandListDestroy(cmdl);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandListDestroy failed\n";
        return;
    }

    ret = zeCommandQueueDestroy(cmdq);
    if (ret != ZE_RESULT_SUCCESS) {
        std::cerr << "zeCommandQueueDestroy failed\n";
        return;
    }
}
...
test_multi_descriptor_offsets(bool p_thread) {
    ...
    void *send_buf_host = nullptr;
    void *recv_buf_host = nullptr;
    int src_device_id = 0;
    int dst_device_id = 1;
    ...
    allocateAndRegister(engine1, src_device_id, VRAM_SEG, send_buf, TOTAL_SIZE, send_md);
    allocateAndRegister(engine2, dst_device_id, VRAM_SEG, recv_buf, TOTAL_SIZE, recv_md);
    ...
    for (int i = 0; i < DESC_COUNT; i++) {
        ...
        for (size_t j = 0; j < DESC_SIZE; j++) {
            ((uint8_t *)send_buf_host)[offset + j] = pattern;
        }
    }

    // from host to device on send agent
    levelZeroMemCpy(send_buf_host, send_buf, TOTAL_SIZE,  devices_list[src_device_id]);
    ...
    levelZeroMemCpy(recv_buf, recv_buf_host, TOTAL_SIZE, devices_list[dst_device_id]);
